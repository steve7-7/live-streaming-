...
export default function Messages({ onCall }: { onCall: (audioOnly: boolean) => void }) {
  ...
  return (
    <div className="mx-auto flex h-[calc(100vh-8rem)] max-w-6xl lg:h-[calc(100vh-4rem)]">
      ...
      <div
        className={cn(
          "w-full border-r border-slate-200 dark:border-slate-800 md:w-80",
          active && "hidden md:block"
        )}
      >
        <div className="flex items-center justify-between p-4">
          <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Messages</h1>
          <button className="rounded-full bg-violet-500 p-2 text-white transition hover:bg-violet-600">
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p
                  className={cn(
                    "truncate text-xs",
                    c.typing ? "text-violet-500 font-medium" : "text-slate-500"
                  )}
                >
      ...
      {active ? (
        <div className="flex flex-1 flex-col">
          <div className="flex items-center gap-3 border-b border-slate-200 dark:border-slate-800 p-3">
            <button
              ...
              className="rounded-full p-2 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 md:hidden"
            >
            ...
            <button
              onClick={() => onCall(true)}
              className="rounded-full p-2.5 text-slate-600 dark:text-slate-300 transition hover:bg-slate-100 dark:hover:bg-slate-800"
            >
          ...
          <div className="flex-1 space-y-2 overflow-y-auto p-4">
            {(threads[active.id] ?? activeMessages ?? []).map((m) => (
              <div key={m.id} className={cn("flex", m.fromMe ? "justify-end" : "justify-start")}>
                <div
                  className={cn(
                    "max-w-[75%] rounded-2xl px-4 py-2 text-sm animate-[slideUp_.2s_ease]",
                    m.fromMe
                      ? "rounded-br-md bg-violet-500 text-white"
                      : "rounded-bl-md bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200"
                  )}
                >
                  {m.text}
                  <span
                    className={cn(
                      "ml-2 text-[10px]",
                      m.fromMe ? "text-white/70" : "text-slate-400"
                    )}
                  >
                    {m.time}
                  </span>
        ...
        <div className="hidden flex-1 flex-col items-center justify-center gap-3 text-slate-400 md:flex">
          <div className="rounded-full bg-violet-50 dark:bg-violet-500/10 p-6">
            <Icon.Chat className="h-10 w-10 text-violet-400" />
          </div>
