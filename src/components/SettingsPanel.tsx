...
function Toggle({ on, onChange }: { on: boolean; onChange: () => void }) {
  return (
    <button
      onClick={onChange}
      className={cn("relative h-6 w-11 rounded-full transition", on ? "bg-violet-500" : "bg-slate-300 dark:bg-slate-600")}
    >
...
export default function SettingsPanel({ open, onClose, camOn, micOn, facing, setCamOn, setMicOn, setFacing }: Props) {
    ...
    <Modal open={open} onClose={onClose} title="Settings" wide>
      <div className="mb-5 flex gap-1 rounded-xl bg-slate-100 dark:bg-slate-800 p-1">
        {tabs.map((t) => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={cn("flex flex-1 items-center justify-center gap-2 rounded-lg py-2 text-sm font-medium transition",
              tab === t.id ? "bg-white dark:bg-slate-700 text-violet-600 dark:text-white shadow" : "text-slate-500 dark:text-slate-400")}>
            {t.icon}{t.label}
          </button>
        ))}
      </div>
      ...
      {tab === "video" && (
        <div className="space-y-5">
          ...
          <div>
            <p className="mb-2 text-sm font-medium text-slate-700 dark:text-slate-200">Camera facing</p>
            <div className="grid grid-cols-2 gap-2">
              {(["user", "environment"] as const).map((f) => (
                <button key={f} onClick={() => setFacing(f)}
                  className={cn("flex items-center justify-center gap-2 rounded-xl border-2 py-3 text-sm font-medium transition",
                    facing === f ? "border-violet-500 bg-violet-50 dark:bg-violet-500/10 text-violet-600 dark:text-violet-300"
          ...
          <div>
            <p className="mb-2 text-sm font-medium text-slate-700 dark:text-slate-200">Resolution</p>
            <div className="grid grid-cols-4 gap-2">
              {["480p", "720p", "1080p", "4K"].map((r) => (
                <button key={r} onClick={() => setResolution(r)}
                  className={cn("rounded-lg border-2 py-2 text-xs font-semibold transition",
                    resolution === r ? "border-violet-500 bg-violet-50 dark:bg-violet-500/10 text-violet-600 dark:text-violet-300"
                      : "border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300")}>{r}</button>
              ))}
            </div>
          </div>
      ...
      {tab === "audio" && (
        <div className="space-y-5">
          ...
          <Slider label="Microphone volume" value={micVol} onChange={setMicVol} icon={<Icon.Mic className="h-4 w-4" />} />
          <Slider label="Speaker volume" value={speakerVol} onChange={setSpeakerVol} icon={<Icon.Volume className="h-4 w-4" />} />
          ...
          <div className="rounded-xl bg-slate-50 dark:bg-slate-800/50 p-4">
            ...
            <div className="flex gap-1">
              {Array.from({ length: 20 }).map((_, i) => (
                <span key={i} className="h-6 flex-1 rounded-sm transition-all"
                  style={{ background: micOn && i < (micVol / 5) ? (i > 15 ? "#ef4444" : i > 11 ? "#f59e0b" : "#10b981") : "#cbd5e1", opacity: micOn ? 1 : 0.3 }} />
              ))}
            </div>
          </div>
        </div>
